...
// NewMemCache initializes the memory cache with the provided configuration.
...
// deleteMemCache Deletes a key from the memCache.
//
// Parameter:
//
...
//
//	bool
func deleteMemCache(m *MemCache, key string) bool {
...
// clearMemCache clears the instances in the memory cache.
func clearMemCache(m *MemCache) {
...
// deleteExpiredAll deletes all expired instances from the MemCache and returns the number of instances deleted.
//
// Parameter:
// m *MemCache - a pointer to the MemCache object.
// Return type:
// int - the number of deleted instances.
func deleteExpiredAll(m *MemCache) int {
	deleted := 0
	for _, instance := range m.instances {
		if instance.IsExpired() {
			if deleteMemCache(m, instance.Key) {
				deleted++
			}
		}
	}
...
// deleteExpired deletes expired instances from the MemCache based on the given size.
//
// Parameters:
// m *MemCache - a pointer to the MemCache object.
// size int - the size parameter for deletion.
func deleteExpired(m *MemCache, size int) {
	deleted := 0
	if count(m) <= size {
		m.ch <- deleteExpiredAll(m)
		return
	}
	...
	for _, randNumber := range tmp {
		if count(m) > randNumber && m.instances[randNumber].IsExpired() {
			if deleteMemCache(m, m.instances[randNumber].Key) {
				deleted++
			}
		}
	}
