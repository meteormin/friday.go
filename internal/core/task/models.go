...
type Job struct {
	gorm.Model
	JobID      string `gorm:"not null;uniqueIndex"`
	Name       string `gorm:"not null"`
	Tags       []JobTag
	JobCounter []JobCounter `gorm:"foreignKey:ParentID"`
	JobTiming  []JobTiming  `gorm:"foreignKey:ParentID"`
}
...
func (j *JobRepositoryImpl) FindByJobID(jobID uuid.UUID) (*Job, error) {
	ent := Job{}
	err := j.db.Where("job_id = ?", jobID).First(&ent).Error
	if err != nil {
		return nil, err
	}
